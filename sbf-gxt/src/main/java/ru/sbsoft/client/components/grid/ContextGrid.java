...
public class ContextGrid extends BaseGrid<Row> {
    ...
    protected ValueProvider<Row, BigDecimal> createKeyProvider() {
        if (getMetaInfo() != null) {
            final Column temporalKeyColumn = getMetaInfo().getTemporalKey();
            if (temporalKeyColumn != null) {
                final ColumnConfig columnConfig = (ColumnConfig) temporalKeyColumn.getData(Column.COLUMN_CONFIG_PREFIX);
