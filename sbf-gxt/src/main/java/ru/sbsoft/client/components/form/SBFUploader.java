...
/**
 * <p> SBFUploader panel. </p>
 *
 * @author Manolo Carrasco MoГ±ino
 *
 * <h3>Features</h3> <ul> <li>Renders a form with an input file for sending the file, and a hidden iframe where is
 * received the server response</li> <li>The user can add more elements to the form</li> <li>It asks the server for the
 * upload progress continuously until the submit process has finished.</li> <li>It expects xml responses instead of
 * gwt-rpc, so the server part can be implemented in any language</li> <li>It uses a progress interface so it is easy to
 * use customized progress bars</li> <li>By default it renders a basic progress bar</li> <li>It can be configured to
 * automatic submit after the user has selected the file</li> <li>It uses a queue that avoid submit more than a file at
 * the same time</li> </ul>
 *
 * <h3>CSS Style Rules</h3> <ul> <li>.GWTUpld { SBFUploader container }</li> <li>.GWTUpld .upld-input { style for the
 * FileInput element }</li> <li>.GWTUpld .upld-status { style for the IUploadStatus element }</li> <li>.GWTUpld
 * .upld-button { style for submit button if present }</li> </ul>
 */
 //TODO not used
public class SBFUploader {}
///* extends Composite implements IsUpdateable, IUploader, HasJsData {
//
//	@Override
...
//
//	/**
//	 * FormPanel add method only can be called once. This class override the add method to allow multiple additions to a
//	 * flowPanel.
//	 */
...
//	public static final IUploader.UploaderConstants I18N_CONSTANTS = GWT.create(IUploader.UploaderConstants.class);
...
//	private static final int DEFAULT_TIME_MAX_WITHOUT_RESPONSE = 5 * 000;// 10 * 60 * 000;
//	private static final int DEFAULT_UPDATE_INTERVAL = 500;
//	private static HashSet<String> fileDone = new HashSet<String>();
//	private static ArrayList<String> fileQueue = new ArrayList<String>();
...
//		SBFUploader.statusInterval = statusInterval;
//	}
//
//	/**
//	 * Configure the maximal time without a valid response from the server. When this period is reached, the upload
//	 * process is canceled.
//	 */
//	public static void setUploadTimeout(int uploadTimeout) {
//		SBFUploader.uploadTimeout = uploadTimeout;
//	}
//
...
//			String url = null;
//			try {
//				url = IUploader.Utils.getXmlNodeValue(XMLParser.parse(text), TAG_BLOBSTORE_PATH);
...
//		}
//	};
//	private ArrayList<IUploader.OnChangeUploaderHandler> onChangeHandlers = new ArrayList<IUploader.OnChangeUploaderHandler>();
...
//			basename = IUploader.Utils.basename(getFileName());
...
//		}
//	};
//	private ArrayList<IUploader.OnFinishUploaderHandler> onFinishHandlers = new ArrayList<IUploader.OnFinishUploaderHandler>();
...
//			hasSession = true;
//			try {
//				String s = IUploader.Utils.getXmlNodeValue(XMLParser.parse(response.getText()), "blobstore");
...
//					+ e.getMessage() + response.getText()
//					: i18nStrs.submitError();
//				cancelUpload(message);
//			}
//		}
//	};
//	private ArrayList<IUploader.OnStartUploaderHandler> onStartHandlers = new ArrayList<IUploader.OnStartUploaderHandler>();
//	private ArrayList<IUploader.OnStatusChangedHandler> onStatusChangeHandlers = new ArrayList<IUploader.OnStatusChangedHandler>();
//	/**
//	 * Handler called when the status request response comes back.
//	 *
//	 * In case of success it parses the xml document received and updates the progress widget In case of a non timeout
//	 * error, it stops the status repeater and notifies the user with the exception.
//	 */
...
//					".*(" + TAG_MSG_START + "|" + LEGACY_TAG_MSG_START + ")([\\s\\S]*?)" + TAG_MSG_END + ".*", "$2");
//				serverResponse = serverResponse.replace(TAG_MSG_LT, "<").replace(TAG_MSG_GT, ">")
...
//			serverInfo.name = IUploader.Utils.getXmlNodeValue(doc, TAG_NAME);
//			serverInfo.ctype = IUploader.Utils.getXmlNodeValue(doc, TAG_CTYPE);
//			String size = IUploader.Utils.getXmlNodeValue(doc, TAG_SIZE);
//			if (size != null) {
//				serverInfo.size = Integer.parseInt(size);
//			}
//			serverInfo.field = IUploader.Utils.getXmlNodeValue(doc, TAG_FIELD);
//			serverInfo.message = IUploader.Utils.getXmlNodeValue(doc, TAG_MESSAGE);
//
...
//	 *
//	 * If the client hasn't got the session, it asks for a new one and the submit process is delayed until the client
//	 * has got it
//	 */
...
//			finished = false;
//			serverResponse = null;
//			serverInfo = new IUploader.UploadedInfo();
//
//			statusWidget.setVisible(true);
...
//	private IUploader.UploadedInfo serverInfo = new IUploader.UploadedInfo();
...
//	protected IUploader.UploaderConstants i18nStrs = I18N_CONSTANTS;
//	private boolean successful = false;
//	private SBFUploader thisInstance;
...
//
//	/**
//	 * Default constructor. Initialize widget components and layout elements using the standard file input.
//	 */
//	public SBFUploader() {
...
//
//	/**
//	 * Initialize widget components and layout elements using the standard file input.
//	 *
//	 * @param automaticUpload when true the upload starts as soon as the user selects a file
//	 */
//	public SBFUploader(boolean automaticUpload) {
...
//	 */
//	public SBFUploader(FileInputType type) {
//		this(type, null);
//	}
//
//	/**
...
//	 * @param automaticUpload when true the upload starts as soon as the user selects a file
//	 */
//	public SBFUploader(FileInputType type, boolean automaticUpload) {
//		this(type);
...
//	 */
//	public SBFUploader(FileInputType type, FormPanel form) {
//		thisInstance = this;
//		this.fileInputType = type;
//
//		if (form == null) {
//			form = new SBFUploader.FormFlowPanel();
//		}
//		uploadForm = form;
...
//	@Override
//	public void add(Widget w, int index) {
//		if (uploadForm instanceof SBFUploader.FormFlowPanel) {
//			((SBFUploader.FormFlowPanel) uploadForm).add(w, index);
//		} else {
//			add(w);
//		}
//	}
...
//	 *
//	 * It is useful in forms where the developer whats the user to submit information but the attachment is optional.
//	 *
...
//
//	/**
//	 * Returns the link for getting the uploaded file from the server It's useful to display uploaded images or generate
...
//
//	/**
//	 * Returns a JavaScriptObject properties with the url of the uploaded file. It's useful in the exported version of
//	 * the library. Because native javascript needs it
//	 */
//	@Override
...
//	 */
//	@Override
//	public void setUploaded(IUploader.UploadedInfo info) {
//		serverInfo = info;
//		successful = true;
...
//	 *
//	 * Override this method if you want to add a customized behavior, but remember to call this in your function
//	 */
...
//
//	/**
//	 * Method called when the file upload process has finished, or the file has been canceled or removed from the queue.
//	 * Override this method if you want to add a customized behavior, but remember to call this in your function.
//	 */
...
//
//	/**
//	 * Method called when the file is added to the upload queue. Override this if you want to add a customized behavior,
...
//			error = IUploader.Utils.getXmlNodeValue(doc, "error");
//		} catch (Exception e) {
...
//			cancelUpload(error);
//			return;
//		} else if (IUploader.Utils.getXmlNodeValue(doc, TAG_WAIT) != null) {
//			if (serverResponse != null) {
//				log("server response received, cancelling the upload " + getFileName() + " " + serverResponse, null);
//				successful = true;
//				uploadFinished();
//			}
//		} else if (IUploader.Utils.getXmlNodeValue(doc, TAG_CANCELED) != null) {
//			log("server response is: canceled " + getFileName(), null);
//			successful = false;
//			canceled = true;
//			uploadFinished();
//			return;
//		} else if (IUploader.Utils.getXmlNodeValue(doc, TAG_FINISHED) != null) {
...
//			}
//			return;
//		} else if (IUploader.Utils.getXmlNodeValue(doc, TAG_PERCENT) != null) {
//			lastData = now();
//			int transferredKB = Integer.valueOf(IUploader.Utils.getXmlNodeValue(doc, TAG_CURRENT_BYTES)) / 1024;
//			int totalKB = Integer.valueOf(IUploader.Utils.getXmlNodeValue(doc, TAG_TOTAL_BYTES)) / 1024;
...
//	 * submits the form.
//	 *
//	 * This is needed because this client application usually is part of static files, and the server doesn't set the
//	 * session until dynamic pages are requested.
//	 *
//	 * If we submit the form without a session, the server creates a new one and send a cookie in the response, but the
//	 * response with the cookie comes to the client at the end of the request, and in the meanwhile the client needs to
//	 * know the session in order to ask the server for the upload status.
//	 */
...
//
//	/**
//	 * Called when the uploader detects that the upload process has finished: - in the case of submit complete. - in the
//	 * case of error talking with the server.
//	 */
...
//		}
//		boolean valid = IUploader.Utils.validateExtension(validExtensions, filename);
//		if (!valid) {
//			finished = true;
