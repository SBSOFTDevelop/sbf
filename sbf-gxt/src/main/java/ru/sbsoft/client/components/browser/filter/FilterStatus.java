...
public class FilterStatus {
    ...
    private void filters(FilterInfoGroup group, int level, BooleanOperator parentBooleanOperator, StringBuilder buffer) {
        ...
        for (final FilterInfo f : childFilters) {
            ...
            if (value == null) {
                ...
                final Column colum = baseGrid.getMetaInfo().getColumnForAlias(f.getColumnName());
                if (colum != null) {
                    CustomColumnConfig config = (CustomColumnConfig) colum.getData(Column.COLUMN_CONFIG_PREFIX);
    ...
    private String getColumnName(String alias) {
        final Columns metaInfo = baseGrid.getMetaInfo();
        final Column c = metaInfo.getColumnForAlias(alias);
        if (c != null) {
