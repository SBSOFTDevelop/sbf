...
import ru.sbsoft.sbf.app.form.controller.*;
 ...
 * @param <MODEL>
 */
public abstract class BaseForm<MODEL extends IFormModel> { // implements ClientUtils.ErrorHandler {

    private final static Map<Window, BaseForm> windows = new HashMap<Window, BaseForm>();

    private final BaseWindow window = new BaseWindow();
    private DefaultContainerController<MODEL> formController;
    private BaseFormSaveController<MODEL> saveController;
    private IBaseFormOwner formOwner;

    private final GwTi18nUnit i18nUnit = GwTi18nUnit.getInstance();
    private final IFormServiceAsync RPC = SBFConst.FORM_SERVICE;
    //
    ...
    private Map<Widget, BaseAdapter> fields = new HashMap<Widget, BaseAdapter>();
    ...
    //
    private final WindowAsyncCallback showFormCallback = new WindowAsyncCallback<MODEL>(window) {
    ...
    public BaseForm(final String caption, final boolean showMenu) {
        ...
        formController = new DefaultContainerController<MODEL>();
        saveController = new BaseFormSaveController<MODEL>();
        if (showMenu) {
    ...
    private void onFinishingInitialization() {
        ...
        saveController.addSaveErrorHandler(new IHandler<SaveErrorEvent<MODEL>>() {
            ...
            public void onHandle(SaveErrorEvent event) {
                ...
                if (caught != null && (caught instanceof OptimisticLockException)) {
...
//        }
    ...
    public void hide() {
        // if (checkSaveChanges(CLOSE_CALLBACK)) {
        close();
        // }
    }
    ...
    protected void alert(Component f) {
        selectActiveTab(f);
    }
    ...
    public void updateFieldsInfo(TabPanel tabPanel) {
    ...
    private void registrFields(HasWidgets hasWidgets, HasWidgets mainControl) {
        Iterator<Widget> iterator = hasWidgets.iterator();
        while (iterator.hasNext()) {
            ...
            if (w instanceof Component) {
                ...
                final BaseAdapter adapter = BaseAdapter.create((Component) w, mainControl);
                if (adapter != null) {
    ...
    public static BaseForm forGxtWindow(Window w) {
