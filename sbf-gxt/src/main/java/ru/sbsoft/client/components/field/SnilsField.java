...
public class SnilsField extends MaskField implements AllowBlankControl {
    ...
    public String format(String value) {
        if (testNumber(value, SNILS.SIGNIFICANT_LENGTH)) {
            ...
            s.append(value.substring(0, 3)).append("-");
            s.append(value.substring(3, 6)).append("-");
            s.append(value.substring(6, 9)).append(" ");
            s.append(value.substring(9, 11));
            return s.toString();
        }
        return null;
    }
    ...
    protected String unconvert(String value) {
        if (SNILS.valid(value)) {
            StringBuilder s = new StringBuilder();
            s.append(value.substring(0, 3));
            s.append(value.substring(4, 7));
            s.append(value.substring(8, 11));
            s.append(value.substring(12, 14));
            return s.toString();
        }
        return null;
    }
