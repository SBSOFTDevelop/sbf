...
public interface IConfigService extends SBFRemoteService {
    ...
    List<ColumnCfg> loadConfiguration(GridContext context);

    void saveConfiguration(GridContext context, List<ColumnCfg> columns);

    FiltersBean loadFilter(GridContext context, StoredFilterPath filterPath);

    void saveFilter(GridContext context, FiltersBean filters, StoredFilterPath filterPath);
