...
import ru.sbsoft.svc.widget.core.client.event.CellChangeEvent;
...
import ru.sbsoft.svc.widget.core.client.event.StoreChangeEvent;
 ...
 */
public class ListView<M, N> extends Component implements HasRefreshHandlers, CellChangeEvent.HasCellChangeHandlers<N, Cell<N>>, StoreChangeEvent.HasStoreChangeHandlers<M, ListStore<M>> {
    ...
    private boolean trackMouseOver = SVC.isMSEdge() || !SVC.isTouch();
    ...
    public HandlerRegistration addCellChangeHandler(CellChangeEvent.CellChangeHandler<N, Cell<N>> handler) {
        return addHandler(handler, CellChangeEvent.getType());
    }

    public HandlerRegistration addStoreChangeHandler(StoreChangeEvent.StoreChangeHandler<M, ListStore<M>> handler) {
        return addHandler(handler, StoreChangeEvent.getType());
    }
    ...
    public void setCell(Cell<N> cell) {
        Cell<N> oldCell = this.cell;
        this.cell = cell;

        if (cell != null) {
            CellWidgetImplHelper.sinkEvents(this, cell.getConsumedEvents());
        }
        CellChangeEvent.fireIfNotEqual(this, oldCell, cell);
    }
     ...
     * <li>{@link #onLoadError(ru.sbsoft.svc.data.shared.loader.LoadExceptionEvent)} is called on load exception.</li>
     * </ul>
     *
    ...
    public void setStore(ListStore<M> store) {
        ListStore<M> oldStore = this.store;
        if (this.store != null) {
        ...
        StoreChangeEvent.fireIfNotEqual(this, oldStore, store);
    }
