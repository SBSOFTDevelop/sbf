...
import ru.sbsoft.svc.dnd.core.client.ListViewDragSource;
import ru.sbsoft.svc.dnd.core.client.ListViewDropTarget;
import ru.sbsoft.svc.messages.client.DefaultMessages;
import ru.sbsoft.svc.widget.core.client.ListView;
import ru.sbsoft.svc.widget.core.client.ListViewLabelFilter;
...
public class DualListField<M, T> extends AdapterField<List<M>> {
    ...
    private VerticalLayoutContainer fromContainer;
    private ListViewLabelFilter<M, T> fromSimpleFilter;
    ...
    public DualListField(ListStore<M> fromStore, ListStore<M> toStore, ValueProvider<? super M, T> valueProvider,
        ...
        fromView = new ListView<>(this.fromStore, valueProvider);
        fromView.setCell(cell);
        fromView.setWidth(125);
        fromContainer = new VerticalLayoutContainer();

        toView = new ListView<>(this.toStore, valueProvider);
        toView.setCell(cell);
        toView.setWidth(125);
        ...
        fromContainer.add(fromView, VLC.FILL);

        panel.add(fromContainer);
        panel.add(buttonBar);
        panel.add(toView);
    ...
    public void useFromFilter(boolean b) {
        if (b) {
            useFromFilter(null);
        } else {
            dropFromFilter();
    ...
    public void useFromFilter(ValueProvider<T, String> labelProvider) {
        if (fromSimpleFilter == null || !Objects.equals(labelProvider, fromSimpleFilter.getLabelProvider())) {
            dropFromFilter();
            fromSimpleFilter = new ListViewLabelFilter<>(fromView, labelProvider);
            fromContainer.insert(fromSimpleFilter, 0, VLC.CONST_M2);
        }
    }

    private void dropFromFilter() {
        if (fromSimpleFilter != null) {
            fromSimpleFilter.setView(null);
            fromContainer.remove(fromSimpleFilter);
            fromSimpleFilter = null;
        }
    }
