...
import ru.sbsoft.svc.widget.core.client.CollapsePanel;
...
public class BorderLayoutContainer extends SimpleContainer implements HasCenterWidget, HasNorthWidget, HasSouthWidget,
  ...
  private native void setCollapsed(ContentPanel panel, boolean collapse) /*-{
		panel.@ru.sbsoft.svc.widget.core.client.ContentPanel::collapsed = collapse;
  }-*/;
  ...
  private void assertLayoutDataFeaturesUsed(String from, IsWidget child, BorderLayoutData layoutData) {
    ...
    assert !layoutData.isCollapsible() || child.asWidget() instanceof ContentPanel : "In order to use layoutData.setCollapsible(true) " + from + "(child, layoutData) a SVC ContentPanel has to be used as the child widget.";
  }
