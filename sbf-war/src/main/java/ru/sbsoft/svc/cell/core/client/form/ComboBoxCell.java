...
import ru.sbsoft.svc.cell.core.client.LabelProviderSafeHtmlCell;
...
import ru.sbsoft.svc.data.shared.Store.StoreFilter;
...
import ru.sbsoft.svc.data.shared.loader.ListLoadConfigBean;
...
import ru.sbsoft.svc.data.shared.loader.PagingLoadConfig;
...
import ru.sbsoft.svc.widget.core.client.event.BeforeQueryEvent;
import ru.sbsoft.svc.widget.core.client.event.CellBeforeSelectionEvent;
import ru.sbsoft.svc.widget.core.client.event.CellSelectionEvent;
...
import ru.sbsoft.svc.widget.core.client.toolbar.PagingToolBar;
...
public class ComboBoxCell<T> extends TriggerFieldCell<T> implements HasBeforeSelectionHandlers<T>,
  ...
  private static boolean isMSEdge = SVC.isMSEdge();
  ...
  protected void initView(ListView<T, ?> listView) {
    ...
    listView.addRefreshHandler(new RefreshEvent.RefreshHandler() {
      ...
      public void onRefresh(RefreshEvent event) {
        if (lastParent != null && (SVC.isIE() || isMSEdge)) {
  ...
  protected void onTriggerClick(Context context, XElement parent, NativeEvent event, T value, ValueUpdater<T> updater) {
    ...
    if (!SVC.isTouch()) { // EXTGWT-4146: don't focus on touch
