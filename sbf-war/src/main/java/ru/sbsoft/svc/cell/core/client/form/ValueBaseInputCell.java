...
import ru.sbsoft.svc.cell.core.client.FocusableCell;
...
import ru.sbsoft.svc.widget.core.client.event.ParseErrorEvent.HasParseErrorHandlers;
...
public abstract class ValueBaseInputCell<T> extends FieldCell<T> implements HasParseErrorHandlers, FocusableCell {
  ...
  protected void onBlur(Context context, XElement parent, T value, NativeEvent event, ValueUpdater<T> valueUpdater) {
    ...
    if (finishEditOnBlur && (SVC.isSafari() || SVC.isChrome() || SVC.isIE9() || SVC.isIE10() || SVC.isMSEdge())) {
  ...
  protected void removeEmptyText(XElement parent) {
    ...
    if (((emptyText != null && emptyText.equals(v)) || "".equals(v)) && (SVC.isIE8() || SVC.isIE9())) {
      setText(parent, "");
      select(parent, 0, 0);
    }
  }
