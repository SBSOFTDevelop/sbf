...
public class ColumnBehavior extends CustomBehavior implements FieldsComboBox.ColumnSource, LookupSuccessHandler {
    ...
    public void setColumn(IColumn column, boolean force) {
        setColumn((CustomColumnConfig) column.getData(IColumn.COLUMN_CONFIG_PREFIX), force);
    }
    ...
    public void initConditions() {
        ...
        for (Condition c : cList) {
            if (!compareFieldsMode || c.isCanCompareFields()) {
    ...
    private class ResoreFiltersAction {
        ...
        public void execute() {
            try {
                IColumn c = restoreField(getFields(), filterInfo.getColumnName(), "");
                setColumn(c, true);
                if (filterInfo instanceof ColumnsFilterInfo) {
                    setCompareFieldsMode(true);
                    IColumn altColumn = restoreField(getFields_II(), ((ColumnsFilterInfo) filterInfo).getColumnName2(), "(2)");
        ...
        public IColumn restoreField(FieldsComboBox cb, String colName, String prefix) throws FilterSetupException {
            IColumns meta = filterItem.getHoldersList().getGrid().getMetaInfo();
            IColumn c = check(meta.getColumnForAlias(colName), I18n.get(SBFBrowserStr.msgColumnNotFound, colName));
            check((CustomColumnConfig) c.getData(IColumn.COLUMN_CONFIG_PREFIX), "ColumnConfig" + prefix);
