...
public class GridUtils {
    ...
    private static boolean isActiveWidget(IsWidget isw) {
        Widget w = isw != null ? isw.asWidget() : null;
        boolean visible = w != null && ((w instanceof Component) ? ((Component)w).isVisible(true) : w.isVisible());
        if (visible) {
            Widget widget = w;
            boolean activeWidget = true;
            while ((widget = widget.getParent()) != null) {
                if (widget.getParent() instanceof CardLayoutContainer) {
                    activeWidget &= ((CardLayoutContainer) widget.getParent()).getActiveWidget() == widget;
                }
            }
            return activeWidget;
        }
    ...
    public static boolean isActiveGrid(SystemGrid grid) {
        return isActiveWidget(grid) || grid.getParent() instanceof LookupGridMenu;
    }

    public static boolean isActiveGrid(Grid<?> grid) {
        return isActiveWidget(grid);
    }
