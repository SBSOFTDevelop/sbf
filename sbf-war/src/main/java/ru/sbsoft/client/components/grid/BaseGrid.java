...
public abstract class BaseGrid<DataInfoModel extends MarkModel> extends SystemGrid<DataInfoModel> implements ClientUtils.ErrorHandler, IGridInfo, IBaseFormOwner {
    ...
    //
    private final List<BaseForm> openedForms = new ArrayList<>();
    private final List<EditFormFactoryInfo> formFactories = new ArrayList<>();
    //
    private ActionManager actionManager;

    private final Map<GridEvent, Action> actionsMap = new EnumMap<>(GridEvent.class);
    //
    ...
    public void setFilters(final FiltersBean filters) {
        CONFIG_RPC.saveFilter(getGridContext(), filters, DEFAULT_FILTER_PATH, new DefaultAsyncCallback<Void>() {
            ...
            public void onResult(Void result) {
                setFilters(DEFAULT_FILTER_PATH);
            }
        });
    }
...
//    }
            ...
            public void onResult(MarkModel result) {
                if (result instanceof Row) {
                    ...
                    if (row.getColumns() == null) {
                        RPC.getMeta(getGridContext(), new DefaultAsyncCallback<IColumns>() {
    ...
    protected void getEditForm(final DataInfoModel model, AsyncCallback<BaseForm> callback) {
        DataInfoModel bindModel = model;
        if (bindModel == null && isClonable()) {
            bindModel = getGrid().getSelectionModel().getSelectedItem();
        }
        createEditForm(bindModel, new AsyncCallback<BaseForm>() {
            @Override
    ...
    public void initEventMap() {
        bindEvent(GridEvent.INVERT_MARK, getMarkCheckAction());
        bindEvent(GridEvent.DOUBLE_CLICK, getDblClickAction());
        bindEvent(GridEvent.DELETE, getDeleteAction());
        bindEvent(GridEvent.NEW, getInsertAction());
        bindEvent(GridEvent.EDIT, getUpdateAction());
        bindEvent(GridEvent.CLONE, getCloneAction());
        bindEvent(GridEvent.EXPORT_XLS, getExportAction());
        bindEvent(GridEvent.REFRESH, getRefreshAction());
    }
...
//        getGrid().getView().focus();
        getGrid().getElement().setTabIndex(-1);
        getGrid().getElement().focus();
    }
}
