...
import ru.sbsoft.svc.dnd.core.client.DND;
import ru.sbsoft.svc.dnd.core.client.DndDragStartEvent;
import ru.sbsoft.svc.dnd.core.client.TreeDragSource;
import ru.sbsoft.svc.dnd.core.client.TreeDropTarget;
...
public class GenericTree<K extends Number> extends AbstractTree<K, TreeNode<K>> implements IBaseFormOwner {
    ...
    protected void initTools() {
        ...
        TreeAction editAct = new EditSelectedTreeAction(SBFBrowserStr.menuOperRowUpdate, SBFResources.BROWSER_ICONS.RowUpdate16(), SBFResources.BROWSER_ICONS.RowUpdate()) {
            @Override
            protected void exec(TreeNode<K> selection) {
                showForm(selection, getParent(selection));
            }
        };
        TreeAction addAct = new EditTreeAction(SBFBrowserStr.menuOperRowInsert, SBFResources.BROWSER_ICONS.RowInsert16(), SBFResources.BROWSER_ICONS.RowInsert()) {
            @Override
            protected void exec(TreeNode<K> selection) {
        ...
        TreeAction delAct = new EditSelectedTreeAction(SBFBrowserStr.menuOperRowDelete, SBFResources.BROWSER_ICONS.RowDelete16(), SBFResources.BROWSER_ICONS.RowDelete()) {
            @Override
        ...
        addToolSep();
        addToolBt(ToolButton.SAVE, editAct);
        addToolBt(ToolButton.PLUS, addAct);
        addToolBt(ToolButton.MINUS, delAct);

        keyActionController.addAction(StdActKey.OPEN, editAct);
        keyActionController.addAction(StdActKey.NEW, addAct);
        keyActionController.addAction(StdActKey.DELETE, delAct);
    }
                                    ...
                                    handleItemEvent(event, new DefaultAsyncCallback<TreeNode<K>>(GenericTree.this) {
                                        ...
                                        public void onResult(TreeNode<K> result) {
                                            ...
                                            if (event.getModel() instanceof TreeNodeSimpleFormModel) {
                                                ...
                                                TreeNodeSimpleFormModel fm = (TreeNodeSimpleFormModel) event.getModel();
                                                TreeNode<K> pn = (fm.getParentNode() == null) ? null : new TreeNodeModel(fm.getParentNode().getID(), fm.getParentNode().getSemanticName(), false);
                                                TreeNode<K> po = getTree().getStore().getParent(result);
                                                //
    ...
    public List<FilterInfo> getParentFilters() {
        if (getTreeFilters().isEmpty()) {
            return Collections.<FilterInfo>singletonList(parentFilterFactory.createFilter(Dict.TREE_PARENT_ID, getTree().getSelectionModel().getSelectedItem()));
        }
        //добавляем фильтры самого дерева
        List<FilterInfo> list = new ArrayList<>();
        list.add(parentFilterFactory.createFilter(Dict.TREE_PARENT_ID, getTree().getSelectionModel().getSelectedItem()));
