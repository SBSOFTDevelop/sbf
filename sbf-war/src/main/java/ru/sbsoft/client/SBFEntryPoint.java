    ...
    public void onModuleLoad() {
        ...
        Window.addWindowClosingHandler(new Window.ClosingHandler() {
            ...
            public void onWindowClosing(Window.ClosingEvent event) {
                WidgetCloseValidalidator wcv = new WidgetCloseValidalidator(RootPanel.get().asWidget());
                if (!wcv.isCloseSafe()) {
                    event.setMessage("Has not saved data. Leave?");
                }
            }
        });
    }
...
//To Do объединить все одноразовые стартовые сервисы в один!!!!
            ...
            updateApplicationInfo(userEnv.getApplicationInfo());
    ...
    protected void updateApplicationInfo(ApplicationInfo result) {
        if (result == null) {
        ...
        SBFConfig.writeString(SBFVariable.APPLICATION_TITLE, result.getApplication());
        browserManager.setApplicationInfo(result);
        refreshAppTitle();
    }
    ...
    protected MainMenuTree getMainMenu() {
        if (mainMenu == null) {
            ...
            mainMenu.addSaveInfoHandler(appInfo -> updateApplicationInfo(appInfo));
        }
        return mainMenu;
    }
    ...
    private void refreshAppTitle() {
        Document.get().setTitle(SBFConfig.readString(SBFVariable.APPLICATION_TITLE));
