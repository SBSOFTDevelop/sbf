...
public class BdExpCellFormat implements IExpCellFormat {
    ...
    public BdExpCellFormat(int precision) {
        this.constPrecision = precision >= 0 ? precision : 0;
    }
    ...
    public String getFormat(Row row) {
        int p = constPrecision;
        if (expr != null) {
            p = getFracDigitsCount(row);
