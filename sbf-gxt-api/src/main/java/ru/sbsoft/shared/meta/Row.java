...
/**
 * Класс представляет строку грида, содержащую коллекцию столбцов, экземпляр класса {@link Columns}.
 * @author balandin
 */
public class Row extends MarkModel {

    private List<?> values;
    private transient Columns columns;
    ...
    public Object getValue(String alias) {
        final Column column = columns.getColumnForAlias(alias);
        if (column == null) {
